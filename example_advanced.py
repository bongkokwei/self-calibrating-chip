...
def example_3_custom_filter():
    ...
    custom_amps = (
        np.sinc(t)
        * np.cos(np.pi * roll_off * t)
        / (1 - (2 * roll_off * t) ** 2 + 1e-10)
    )
...
def example_4_mzi_analysis():
    ...
    for tap_idx in range(8):
        ax1.plot(
            mzi_phases, tap_powers[:, tap_idx], label=f"Tap {tap_idx+9}", linewidth=1.5
        )
    ...
    for phase in phases_to_show:
        ...
        ax2.bar(
            np.arange(8) + phases_to_show.index(phase) * 0.15,
            tap_powers[idx, :],
            width=0.15,
            label=f"φ = {phase:.2f} rad",
            alpha=0.7,
        )
...
def example_5_filter_comparison():
    ...
    filters = {
        ...
        "Blackman": np.sinc(n - center)
        * (0.42 - 0.5 * np.cos(2 * np.pi * n / 7) + 0.08 * np.cos(4 * np.pi * n / 7)),
    }
...
def example_6_phase_response_analysis():
    ...
    for idx, (name, phases) in enumerate(phase_configs.items()):
        ...
        # Plot phase
        line2 = ax_twin.plot(
            freq_range / 1e9, np.angle(H), "r-", linewidth=1.5, label="Phase"
        )
